  ...
  if (confirm("Clique em OK para confirmar a operação.")) {
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        document.getElementById("msg-php").innerHTML =
          "Erro na execucao do Ajax";
      }
    };
    xhttp.open("POST", "crud.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function salvarForm() {
  ...
  xhttp.send(formData);
  if (xhttp.status != 200) {
    var frmCrud = document.getElementById("frmCrud");
    var onSuccessRedirect = frmCrud.dataset.onSuccessRedirect;
